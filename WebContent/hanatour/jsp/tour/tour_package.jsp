...
<%
	// 파라미터 받기
	int packageIdx = Integer.parseInt(request.getParameter("package_idx")); // 카테고리에서 이동시 파라미터
	...
	ArrayList<CategoryFlagDto> categoryFlag = cDao.categoryFlag(countryCity);
	
	ArrayList<PackageTopOneLineInfoDto> ptoliDto =pdao.packageTopOneLineInfo(packageIdx);
	ArrayList<String> topImg = pdao.packageTopImg(packageIdx);
	ArrayList<String> topTag = pdao.packageTopTag(packageIdx);
	ArrayList<PackageTwoFlightInfoDto> twoFlightDto = pdao.packageTwoFlightInfo(packageIdx);
	ArrayList<PackageOneFlightInfoDto> oneFlightDto = pdao.packageOneFlightInfo(packageIdx);
	ArrayList<PackageReservationInfoDto> packageReservation = pdao.packageReservationInfo(packageIdx);
	ArrayList<PackagePriceDto> priceDto = pdao.packagePrice(packageIdx);
	ArrayList<PackageInfoDto> packageInfo = pdao.packageInfo(packageIdx);
	ArrayList<PackageContainInfoDto> packageContainInfoDto = pdao.packageContainInfo(packageIdx);
	ArrayList<PackageReservationNoticeDto> packageReservationNotice = pdao.packageReservationNotice(packageIdx);
	ArrayList<PackageMeetingInfoDto> packageMeetingInfoDto = pdao.packageMeetingInfo(packageIdx);
	ArrayList<HotelAttractionAttractionDto> hotelAttractionAttraction = pdao.HotelAttractionAttraction(packageIdx);
	ArrayList<TabSmallHotelBoxDto> tabSmallHotelBox = pdao.tabSmallHotelBox(packageIdx);
	ArrayList<TabHotelInfoDto> tabHotelInfoDto = pdao.tabHotelInfo(packageIdx);
	ArrayList<HotelAmenityDto> hotelAmenity = pdao.hotelAmenity();
	ArrayList<HotelFacilityDto> hotelFacility = pdao.hotelFacility();
	
	String date = pdao.countDay(hotelAttractionAttraction.get(0).getAttractionDate()); // 날짜 계산
	
	ArrayList<SafetyInfoDto> safetyInfo = pdao.safetyInfo(packageIdx);
	ArrayList<OptionalTourDto> optionalTourContent =pdao.optionalTourContent(packageIdx);
	ArrayList<OptionalTourDto> optionalTourTitle =pdao.optionalTourTitle(packageIdx);
	ArrayList<ScheduleTitleDto> scheduleTitle = pdao.scheduleTitle(packageIdx);
	...
	ArrayList<ScheduleOrderDto> scheduleOrder = pdao.scheduleOrder(packageIdx);
	ArrayList<Type1FlightDto> type1Flight = pdao.type1Flight(packageIdx);
	ArrayList<Type2CityDto> type2City = pdao.type2City(packageIdx);
	ArrayList<Type3ThinTextDto> type3ThinText = pdao.type3ThinText(packageIdx);
	ArrayList<Type4BoldTextDto> type4BoldText = pdao.type4BoldText(packageIdx);
	ArrayList<Type5AttractionDto> type5Attraction = pdao.type5Attraction(packageIdx);
	ArrayList<Integer> type5AttractionMany = pdao.type5AttractionMany(packageIdx);
	ArrayList<Type5AttractionImgDto> type5AttractionImg = pdao.type5AttractionImg(packageIdx);
	ArrayList<Type6FoodDto> type6Food = pdao.type6Food(packageIdx);
	ArrayList<Type6FoodImgDto> type6FoodImg = pdao.type6FoodImg(packageIdx);
	ArrayList<Type8NoticeDto> type8Notice = pdao.type8Notice(packageIdx);
	ArrayList<Type8NoticeImgDto> type8NoticeImg = pdao.type8NoticeImg(packageIdx);
	ArrayList<Type9InnDto> type9Inn = pdao.type9Inn(packageIdx);
	ArrayList<Type10MealDto> type10Meal = pdao.type10Meal(packageIdx);
	// 리뷰 정보 기준 dao = packageReviewContent(1);
	ArrayList<PackageReviewContentDto> packageReviewContent = pdao.packageReviewContent(packageIdx, pageNum);
	ArrayList<PackageReviewImgDto> packageReviewImg = pdao.packageReviewImg();
	ArrayList<PackageReviewTagDto> packageReviewTag = pdao.packageReviewTag();
	ArrayList<String> reviewCategoryPackage = pdao.reviewCategoryPackage(packageIdx);
	// 후기 idx 최대 숫자
	int maxReviewIdx = pdao.maxReviewIdx();
...
<head>
	...
	<script>
	$(function() {
	    let page_num = <%=pageNum%>; // 서버에서 페이지 번호를 올바르게 전달받아야 합니다.
	    let package_idx = <%=packageIdx%>; // 패키지 인덱스를 서버에서 전달받아야 합니다.

	    // 페이지가 로드되면 review 함수 호출
	    review(page_num, package_idx);

	    function review(page, packageIdx) {
	        $.ajax({
	            type: 'get',
	            data: { page_num: page, package_idx: packageIdx },
	            dataType: "json",
	            url: "/ReviewPageServlet",
	            success: function(res) {
	                let str = ''; // HTML 문자열을 저장할 변수

	                res.reviews.forEach(dto => {
	                    let tagsHtml = '';
	                    ...
	                    dto.tags.forEach(tag => {
	                        switch(tag.text) {
	                            case "호텔이 시내에 있어요":
	                                tagsHtml += '<div class="fl bottom_tag"><span class="down_town_hotel_tag"></span>' + tag.text + '</div>';
	                                break;
	                            case "가이드가 전문적이에요":
	                                tagsHtml += '<div class="fl bottom_tag"><span class="pro_guide"></span>' + tag.text + '</div>';
	                                break;
	                            default:
	                                tagsHtml += '<div class="fl bottom_tag"><span class="default_tag"></span>' + tag.text + '</div>';
	                                break;
	                        }
	                    });
	                    ...
	                    dto.images.forEach(img => {
	                        ...
	                        if (imgUrl && imgUrl.startsWith("http")) {
	                            imagesHtml += '<img class="fl" src="' + imgUrl + '" alt="" />';
	                        } else {
	                            imagesHtml += '<img class="fl" src="/upload/img/' + imgUrl + '" alt="" />';
	                        }
	                    });

	                    // 현재 리뷰에 대한 HTML을 생성합니다.
	                    str += `
	                        <div class="review_box">
	                            <div>
	                                <div class="fl purple_font"><span class="one_purple_star"></span>${dto.reviewStar}</div>
	                                <div class="fl person_img"></div>
	                                <div class="fl">${dto.userId}</div>
	                                <div class="fl review_grey_dot"></div>
	                                <div class="fl">${dto.pressLike}</div>
	                                <div class="fl review_grey_dot"></div>
	                                <div class="fl">${dto.age}</div>
	                                <div class="fl review_grey_dot"></div>
	                                <div class="fl">${dto.writeDate}</div>
	                                <div style="clear:both;"></div>
	                            </div>
	                            <div>
	                                <div class="fl grey_font"><strong>상품명</strong></div>
	                                <div class="fl">${dto.packageName}</div>
	                                <div style="clear:both;"></div>
	                            </div>
	                            <div>${dto.text}</div>
	                            <div>${tagsHtml}<div style="clear:both;"></div></div>
	                            <div>${imagesHtml}<div style="clear:both;"></div></div>
	                            <div><button type="button">도움이 돼요</button></div>
	                        </div>
	                    `;
	                });

	                // HTML을 #review_box 요소에 추가
	                $("#review_box").append(str);
	            },
...
<body>
    ...
    <div id="category_body" class="center">
		...
		<div id="main_right" class="fr">
			<form action="<%=request.getContextPath() %>/hanatour/jsp/tour/tour_oversea_reservation.jsp" id="amount_select">
				<div>
					<div>인원선택</div>
				</div>
				<div>
